...
```
LakeMindServer/
  docker-compose.yml          # SeaweedFS + Gravitino + Dragonfly 单机编排
  .env.example                # 镜像/端口/凭据/内存（复制为 .env 后按需修改）
  config/
    seaweedfs/s3.json         # S3 网关身份
    versions.yaml             # 版本配套表
  scripts/
    verify_services.py        # 平台集成验证（S3 / Dragonfly / Gravitino->S3）
...
| 短期记忆 | Dragonfly | 会话状态→多轮更新→任务锁(NX)→缓存 |
...
| [使用手册](docs/usage.md) | 日常操作：启停、S3/Gravitino/Dragonfly 访问、数据管理 |
...
| Apache Gravitino | 8090/9001 | 统一元数据（Iceberg + Fileset） |
| Dragonfly | 6379 | 短期/工作记忆（TTL KV） |

嵌入式引擎（PyIceberg / Lance+LanceDB / DuckDB / Daft）在 Agent 进程内运行，不起容器。
Ray / Apache Ranger / Trino 不在 MVP，生产阶段再引入。
