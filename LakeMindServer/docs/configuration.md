...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DRAGONFLY_IMAGE` | `docker.dragonflydb.io/dragonflydb/dragonfly:latest` | 镜像地址 |
| `DRAGONFLY_PORT` | 6379 | Redis 兼容端口 |
| `DRAGONFLY_PASSWORD` | （空） | 访问密码。**生产环境必须设置** |
...
- **Dragonfly**：使用 `DRAGONFLY_HOST` / `DRAGONFLY_PORT` / `DRAGONFLY_PASSWORD`
...
- `lakemind-dragonfly:6379`（Redis 协议）
...
Gravitino 启动前会先启动 SeaweedFS。Dragonfly 无依赖，独立启动。
