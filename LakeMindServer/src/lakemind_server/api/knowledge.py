...
async def ingest_knowledge(request: Request):
    ...
    return await asyncio.to_thread(KnowledgeService.ingest, ctx, **body)
...
async def search_knowledge(request: Request):
    ctx = get_security_context(request)
    body = await request.json()
    return await asyncio.to_thread(KnowledgeService.search, ctx, **body)


@router.get("/concepts/{concept_id}")
async def get_concept(concept_id: str, request: Request):
    ctx = get_security_context(request)
    return await asyncio.to_thread(KnowledgeService.get_concept, ctx, concept_id)
...
async def list_concepts(request: Request):
    ...
    return await asyncio.to_thread(
        KnowledgeService.list_concepts,
        ctx,
...
async def reindex_knowledge(asset_id: str, request: Request):
    ctx = get_security_context(request)
    return await asyncio.to_thread(KnowledgeService.reindex, ctx, asset_id)
