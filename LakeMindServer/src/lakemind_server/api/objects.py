...
async def delete_object(bucket: str, key: str, request: Request):
    await asyncio.to_thread(_eng(request).delete, bucket, key)
    return {"status": "ok"}


@router.get("/{bucket}")
async def list_objects(bucket: str, request: Request, prefix: str = "", limit: int = 1000):
    keys = await asyncio.to_thread(_eng(request).list, bucket, prefix, limit)
