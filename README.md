...
│  │  Lance/LanceDB  │  │  Daft (DataFrame)│  │  Memory Engine  │          │
│  │  Dragonfly (KV) │  │                 │  │                 │          │
│  │  PostgreSQL     │  │                 │  │                 │          │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘          │
└─────────────────────────────────────────────────────────────────────────┘
...
| **数据层 (DATA)** | 多模态数据底座，透传不做语义解释 | DataMCP | 数据是什么、存哪、怎么读写。Iceberg 表、Lance 向量、S3 文件、Dragonfly KV、PG 图——全量透传给 Steward 和高级 Agent |
...
|-----|------|-------|------|--------|------|
| **LakeMindAssetMCP** | 8401 | `asset` | 业务 Agent | 11 tools, 7 resources | 认知资产面：知识检索/摄入、技能管理、记忆读写、本体查询 |
| **LakeMindDataMCP** | 8402 | `data` | Steward / 高级 Agent | 13 tools | 数据面：Iceberg/DuckDB/LanceDB/S3/Dragonfly/Graph 全量透传 |
| **LakeMindAdminMCP** | 8403 | `admin` | Steward | 15 tools | 管理面：用户/租户/Token/资产类型/平台健康 |
...
| KV 缓存 | **Dragonfly** | Redis 兼容 TTL KV，短期/工作记忆 |
...
| DataFrame | **Daft** | 嵌入式 DataFrame（规划中） |

**认知计算引擎**：

| 引擎 | 选型 | 用途 |
|------|------|------|
| Embedding | **fastembed** | ONNX + BAAI/bge-small-en-v1.5，dim=384，CPU ~2ms/text |
| LLM 网关 | **GatewayLLM** | 多 provider 路由 + fallback，支持 OpenAI/DeepSeek/Anthropic/Ollama |
| 记忆引擎 | **BasicMemory** | 短期 Dragonfly TTL + 长期 Lance 向量双表设计 |
...
| 验证套件 | 脚本 | 结果 |
|----------|------|------|
| REST API (11 域) | `scripts/verify_api.py` | ✅ 104/104 PASS |
| 三 MCP 联合 | `scripts/verify_three_mcp.py` | ✅ 22/22 PASS |
| 全量功能 | `scripts/test_full_suite.py` | ✅ 69/69 PASS |
...
| lakemind-asset-mcp | 8401 | 认知资产面 (11 tools) |
| lakemind-data-mcp | 8402 | 数据面 (13 tools) |
| lakemind-admin-mcp | 8403 | 管理面 (15 tools) |
