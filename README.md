...
| **LakeMindDataMCP** | 8402 | `data` | Steward / 高级 Agent | 18 tools, 6 resources, 2 prompts | 数据面：Iceberg/DuckDB/LanceDB/S3/Valkey/Graph 通过LakeMind的统一API进行访问 |
...
**REST API 网关**：LakeMindServer 提供 40+ OpenAPI 路径的统一 REST API，MCP 通过 httpx 连接池调用，支持 11 引擎健康检查、Bearer 认证、租户/Agent/Scope 三级上下文。外部访问统一通过REST API来访问，不直接访问底层引擎，以保障访问的规范化和便捷化。
