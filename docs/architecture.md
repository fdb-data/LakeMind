...
│  │  Lance/LanceDB  │  │  Daft (规划中)  │  │  Memory Engine  │          │
│  │  Dragonfly (KV) │  │                 │  │                 │          │
│  │  PostgreSQL     │  │                 │  │                 │          │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘          │
└─────────────────────────────────────────────────────────────────────────┘
...
| **Memory** | remember, recall, forget | Dragonfly 短期 + Lance 长期 | remember, recall, forget |
| **Ontology** | query, update | PG graph_nodes/edges | query_ontology, update_ontology |

### 数据层 (DATA)

多模态数据底座，透传不做语义解释。数据是什么、存哪、怎么读写。Iceberg 表、Lance 向量、S3 文件、Dragonfly KV、PG 图——全量透传给 Steward 和高级 Agent。
...
|-----|------|-------|------|--------|------|
| LakeMindAssetMCP | 8401 | `asset` | 业务 Agent | 11 tools, 7 resources | 认知资产面 |
| LakeMindDataMCP | 8402 | `data` | Steward / 高级 Agent | 13 tools | 数据面全量透传 |
| LakeMindAdminMCP | 8403 | `admin` | Steward | 15 tools | 管理面 |
...
| KV 缓存 | Dragonfly | TTL KV | Redis / Valkey |
...
| 引擎 | 选型 | 用途 | 可插拔替换 |
|------|------|------|-----------|
| 即席 SQL | DuckDB | 进程内轻量 SQL | Daft |
| 分布式计算 | Ray 2.41 | 3 节点 12 CPU | — |
...
| 引擎 | 选型 | 用途 | 可插拔替换 |
|------|------|------|-----------|
| Embedding | fastembed | ONNX + BAAI/bge-small-en-v1.5, dim=384 | 外部 API / TEI |
| LLM 网关 | GatewayLLM (自建) | 多 provider 路由 + fallback | LiteLLM |
| 记忆引擎 | BasicMemory | 短期 Dragonfly TTL + 长期 Lance 向量 | mem0 |
