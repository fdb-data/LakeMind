...
def register(mcp, engines: Engines, redact_keys: list[str]) -> None:
    ...
    async def remember(
        ...
        engines.dragonfly.remember(
            ctx, short_key, {"content": content, "context": context or {}, "kind": kind}, ttl
        )
        ...
        return {"memory_id": mem_id, "short_term_key": short_key, "lance_uri": lance_uri, "kind": kind}
    ...
    async def recall(query: str, limit: int = 5, kind: str | None = None) -> dict[str, Any]:
        ...
        flt = f"kind = '{kind}'" if kind else None
        hits = engines.lancedb.search(ctx, MEM_TABLE, qvec, limit, flt)
        memories = [
            {
